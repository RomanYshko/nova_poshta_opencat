									...
									<div class="form-group">
										...
										<div class="col-sm-9">
											<select name="delivery_payer" id="input-delivery_payer" class="form-control">
												...
												<?php } elseif ($payer_type['Ref'] == $delivery_payer) { ?>
...
<script type="text/javascript"><!--
	function validateForm(element) {
		...
		$.ajax( {
			url: 'index.php?route=shipping/novaposhta/getCNForm&token=<?php echo $token; ?>',
			type: 'POST',
			data: post_data,
			dataType: 'json',
	...
	$( function () {
		...
		$('#input-recipient_city_name').autocompleteOM( {
			source: function(request, response) {
                var post_data;
		...
		// Search warehouse
		$('#input-recipient_warehouse_name').autocompleteOM( {
			source: function(request, response) {
				$.ajax( {
					url: 'index.php?route=shipping/novaposhta/autocomplete&token=<?php echo $token; ?>',
					type: 'POST',
		...
		// Search street
		$('#input-recipient_street_name').autocompleteOM( {
			source: function(request, response) {
				$.ajax( {
					url: 'index.php?route=shipping/novaposhta/autocomplete&token=<?php echo $token; ?>',
					type: 'POST',
		...
		$('#button-save').on('click', function () {
			...
			$.ajax( {
				url: 'index.php?route=shipping/novaposhta/saveCN&order_id=<?php echo $order_id; ?>&cn_ref=<?php echo $cn_ref; ?>&token=<?php echo $token; ?>',
				type: 'POST',
				data: $post_data,
				dataType: 'json',
				beforeSend: function() {
				...
				success: function(json) {
					if (json['success']) {
						location.href = 'index.php?route=shipping/novaposhta/getCNList&filter_departure_date_from=' + json['success'] + '&token=<?php echo $token; ?>';
					} else {
						$('.help-block').remove();
